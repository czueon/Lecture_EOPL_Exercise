...
apply_cont (Try_Cont var handler_exp env cont) val = apply_cont cont val

apply_cont (Raise1_Cont cont) val = apply_handler val cont
-- In throw exp, the expression exp evaluates to some error code(val).
-- What you need to do is to raise the error code!
...
-- handle the thrown exception(val) by handler_exp
...
value_of_k (Const_List_Exp nums) env cont = apply_cont cont (List_Val [ Num_Val num | num <- nums ])
...
value_of_k (Proc_Exp var body) env cont =
  let val1 = Proc_Val (Procedure var body env) 
  in apply_cont cont val1
...
value_of_program exp =
  value_of_k exp initEnv End_Cont
